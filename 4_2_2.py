...
class Game:
    def __init__(self, username):
        ...
        self.feeding_button = Button(75, 300, 300, 125, "Feed", (0, 128, 0))
        self.feeding_state = "default"
        self.training_button = Button(425, 300, 300, 125, "Training", (0, 128, 0))
        self.training_state = "default"
        self.resting_button = Button(75, 450, 300, 125, "Rest", (0, 128, 0))
        self.resting_state = "default"
        self.racing_button = Button(425, 450, 300, 125, "Race", (0, 128, 0))
        self.racing_state = "default"
    ...
    def draw(self):
        ...
        # Draw Game Buttons
        if not self.is_in_game_menu:
            self.resting_button.draw()
            self.racing_button.draw()
            self.feeding_button.draw()
            self.training_button.draw()
        
            # Feeding Options
            if self.feeding_state == "options":
                feed_hay_button.draw()
                feed_bran_button.draw()
                feed_hc_button.draw()
                
            # Training Options
            if self.training_state == "options":
                training_strength_button.draw()
                training_acceleration_button.draw()
                training_endurance_button.draw()
...
class HorseSelection:
    def __init__(self):
        self.option_one = Button(50, 500, 200, 50, "Horse 1", (0, 128, 0))
        self.option_two = Button(300, 500, 200, 50, "Horse 2", (0, 128, 0))
        self.option_three = Button(550, 500, 200, 50, "Horse 3", (0, 128, 0))
        
    def draw(self):
        font = pygame.font.Font(None, 36)
        text = font.render("Choose a horse!", True, (255,255,255))
        text_rect = text.get_rect(center=(400, 100))
        screen.blit(text, text_rect)
       
        horse_one_height = 250
        horse_one_width = 200
        horse_one_xpos = 50
        horse_one_ypos = 400
        pygame.draw.rect(screen, (0, 128, 0), (horse_one_xpos, horse_one_ypos, horse_one_width, horse_one_height))
        
        self.option_one.draw()
        self.option_two.draw()
        self.option_three.draw()
...
# Game Buttons
# Food Buttons
feed_hay_button = OptionButton(75, 300, 300, 37, "Hay", (128, 128, 128))
feed_bran_button = OptionButton(75, 344, 300, 37, "Bran", (128, 128, 128))
feed_hc_button = OptionButton(75, 388, 300, 37, "High Calorie", (128, 128, 128))

# Training Buttons
training_strength_button = OptionButton(425, 300, 300, 37, "Strength", (128, 128, 128))
training_acceleration_button = OptionButton(425, 344, 300, 37, "Acceleration", (128, 128, 128))
training_endurance_button = OptionButton(425, 388, 300, 37, "Endurance", (128, 128, 128))
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN:
            if current_state == HORSE_SELECTION:
                select = False
                if horse_select.option_one.is_clicked(event.pos):
                    select = True
                    print("Option One Clicked")
                elif horse_select.option_two.is_clicked(event.pos):
                    select = True
                    print("Option Two Clicked")
                elif horse_select.option_three.is_clicked(event.pos):
                    select = True
            ...
            if not game.is_in_game_menu:
                if current_state == MAIN_GAME:
                    if game.feeding_button.is_clicked(event.pos):
                        game.feeding_state = "options"
                        game.training_state = "default"
                        if feed_hay_button.is_clicked(event.pos):
                            print("Hay clicked")
                        elif feed_bran_button.is_clicked(event.pos):
                            print("Bran clicked")
                        elif feed_hc_button.is_clicked(event.pos):
                            print("High Calorie clicked")
                    elif game.training_button.is_clicked(event.pos):
                        game.training_state = "options"
                        game.feeding_state = "default"
                        if training_acceleration_button.is_clicked(event.pos):
                            print("Acceleration Training")
                        elif training_endurance_button.is_clicked(event.pos):
                            print("Endurance Training")
                        elif training_strength_button.is_clicked(event.pos):
    ...
    if current_state == MAIN_MENU:
    ...
    elif current_state == HORSE_SELECTION:
        font = pygame.font.Font(None, 50)
        text = font.render("Choose a horse!", True, (255,255,255))
        text_rect = text.get_rect(center=(400, 100))
        screen.blit(text, text_rect)
        horse_select.option_one.draw()
        horse_select.option_two.draw()
        horse_select.option_three.draw()
        ...
        if game.feeding_state == "options":
            feed_hay_button.draw()
            feed_bran_button.draw()
            feed_hc_button.draw()
        #else:
            #game.feeding_button.draw()
            
        if game.training_state == "options":
            training_strength_button.draw()
            training_acceleration_button.draw()
            training_endurance_button.draw()
        #else:
            #game.training_button.draw()
